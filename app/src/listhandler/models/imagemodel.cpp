...
ImageModel::ImageModel(FileList *list, QObject *parent) : QAbstractListModel(parent), m_fileList(list) {}
...
void ImageModel::refreshModel() {
    ...
    for (int i = 0; i < size; i++) m_model.append("file:///" + *m_fileList->at(i)->path());

    endResetModel();
}

void ImageModel::browse(const int index) { startExplorer(m_model[index]); }

int ImageModel::size() { return m_model.size(); }
