...
{
    if (actual_direction == direction) return;
...
template <class T>
void ListSort::sortList(FileList* fileList, QMap<int, T*>& map)  // static
{
    // Remove duplicates
    QSet<T*> set;
    foreach (T* value, map.values()) set.insert(value);
    QList<T*> list;
    foreach (T* value, set) list.append(value);
    ...
    foreach (T* value, list) {
        ...
        foreach (int i, indexList) newList.append(fileList->at(i));
    }
...
template <class T>
bool ListSort::sortFunction(T* left, T* right)  // static
{
    bool state;
