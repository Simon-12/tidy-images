...
BaseFile::FileFormat ImageFile::readFormat(QString name, QString extension)
{
    ...
    // WARNING with: qt.gui.icc: fromIccProfile: failed minimal tag size sanity
    // Set dimensions
    QImage img(m_path);
    m_dimensions = QString("%1x%2")
            .arg(img.width())
            .arg(img.height());
...
{
    if(plugin) return plugin->readDate(m_path);
    else
...
void ImageFile::log()
{
    qInfo() << "Read image:" << m_name << " - Labels:" << m_labels;
}
