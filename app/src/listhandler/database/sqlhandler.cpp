...
void SqlHandler::printTable()
{
    ...
    query.prepare("SELECT a.Name, b.Folder, GROUP_CONCAT(c.Label, '; ') AS Labels, "
...
void SqlHandler::addFile(FilePtr file, QString path)
{
    ...
    // Add file
    query.prepare("INSERT INTO Files (Name, Folder_Id, Path, Type, Date, Favorite, Filetype, Duration)"
...
void SqlHandler::createTable()
{
    ...
    // Labels table
    query.prepare("CREATE TABLE Labels ("
                    "Id	INTEGER NOT NULL UNIQUE,"
                    "Label TEXT,"
                    "PRIMARY KEY(Id)"
                    ");");
    ...
    query.prepare("CREATE TABLE Files_Labels ("
