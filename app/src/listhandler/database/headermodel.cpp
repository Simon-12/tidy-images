...
HeaderModel::HeaderModel(QList<QString> &header, QObject *parent) : QAbstractListModel(parent) {
    beginInsertRows(QModelIndex(), 0, header.size());
    foreach (QString item, header) m_items.append(HeaderItem(item));
    endInsertRows();
}
...
int HeaderModel::size() const { return m_items.size(); }
    ...
    foreach (HeaderItem item, m_items) {
        ...
        if (item.direction() != HeaderDirection::Disabled) {
            ...
            switch (item.direction()) {
                case HeaderDirection::Low:
                    direction += " DESC ";
                    break;
                case HeaderDirection::High:
                    direction += " ASC ";
                    break;
                default:
                    break;
