...
TableModel::TableModel(FileList* list, int threshold, QObject *parent) :
...
QVariant TableModel::data(const QModelIndex &index, int role) const
{
    ...
    switch(role)
    {
    case TextRole: return m_tableText[index.column()][index.row()];
    case ColorRole: return *m_tableColor[index.column()][index.row()];
...
QVector<QString> TableModel::setFileColumn(int index)
{
    QVector<QString> list(m_header); // Allocate default size
...
{
    // Set default colors
    QVector<QString*> vec(m_rows, &m_colorDefault);
    m_tableColor.fill(vec, m_columns);
...
template<class T> QString *TableModel::processColor(T &left, T &right)
{
    if(left == right) return &m_colorEqual;
    if(left > right) return &m_colorMarked;
    else return &m_colorDefault;
}
