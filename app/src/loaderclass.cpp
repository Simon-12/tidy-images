...
LoaderClass::LoaderClass(int buffer, QObject *parent) : QObject(parent)
{
    m_bufferSize = buffer;
}
...
void LoaderClass::initLoader(AppMode mode)
{
    ...
    case AppMode::Video:    m_text = "Load Files";    break;
    case AppMode::Database: m_text = "Load Database"; break;
    case AppMode::Image:
    case AppMode::Compare:  m_text = "Load Images";   break;
    }
...
void LoaderClass::setDialog(bool open)
{
    ...
    setProgress(0);
    m_size = -1;   emit sizeChanged();
    m_open = open; emit openChanged();
}
