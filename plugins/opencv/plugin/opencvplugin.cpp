...
VisionPlugin::Frame OpenCVPlugin::videoFrame(const QString &path, double position) {
    // Open the video file
    cv::VideoCapture cap(path.toStdString(), cv::CAP_FFMPEG);  // Open and record video file or stream using the FFMPEG library.
    if (!cap.isOpened()) {
...
bool OpenCVPlugin::initMatching(const QString &original, int matchingOption) {
    ...
    if (m_keypoints.empty() || m_descriptor.empty()) return false;

    return true;
}

double OpenCVPlugin::matchImage(const QString &path, bool printResults) {
    // Read in image
    cv::Mat image;
    ...
    if (good_matches.empty()) return 0.0;
    ...
    //     // Draw matches
    //     cv::Mat matches;
    //     cv::drawMatches(m_original, m_keypoints, image, keypoints, good_matches, matches, cv::Scalar::all(-1),
